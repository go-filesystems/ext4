...
func TestOpenReaderSeesWhatOpenSees(t *testing.T) {
	...
	fs, err := Format(img, 64<<20, FormatConfig{Label: "READER"})
	if err != nil {
		t.Fatal(err)
	}
	body := bytes.Repeat([]byte("ext4 through a reader "), 500)
...
func TestOpenReaderRefusesWritesItCannotDo(t *testing.T) {
	img := filepath.Join(t.TempDir(), "disk.img")
	fs, err := Format(img, 64<<20, FormatConfig{Label: "RO"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestOpenReaderDoesNotCloseTheCallersReader(t *testing.T) {
	img := filepath.Join(t.TempDir(), "disk.img")
	fs, err := Format(img, 64<<20, FormatConfig{})
	if err != nil {
		t.Fatal(err)
	}
	fs.Close()
...
func TestOpenReaderRefusals(t *testing.T) {
	...
	// Something that is not a ext4 image is refused by the same parser Open
	// uses, not by a second opinion.
	if _, err := OpenReader(bytes.NewReader(make([]byte, 4096)), 4096); err == nil {
		t.Error("OpenReader accepted an image with no superblock in it")
	}
}

// Resizing through a reader is refused rather than emulated: the size of the
// caller's reader is the caller's business, and doing nothing here would
// report a resize that did not happen.
func TestOpenReaderRefusesToResize(t *testing.T) {
	d := &readerDevice{r: bytes.NewReader(make([]byte, 4096)), size: 4096}
	if err := d.Truncate(8192); !errors.Is(err, ErrReadOnlyReader) {
		t.Errorf("Truncate = %v, want a refusal", err)
	}
	if n, err := d.Size(); err != nil || n != 4096 {
		t.Errorf("Size = %d, %v", n, err)
	}
	// Sync is nothing to do for a reader with no Sync of its own, rather than
	// an error: a caller that cannot write has nothing to flush.
	if err := d.Sync(); err != nil {
		t.Errorf("Sync over a plain reader = %v", err)
	}
}
